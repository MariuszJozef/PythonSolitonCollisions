...
def CouplingBB(i, j, k):
    return CouplingB(i, j, k) * (waveNumber[i] + waveNumber[j] + waveNumber[k])

def CouplingBBB(i, j, k):
    return CouplingB(i, j, k) * (waveNumber[i] + waveNumber[j] + waveNumber[k])**2

def CouplingC(i, j, k, l):
    return CouplingA(i, j) * CouplingA(i, k) * CouplingA(i, l) \
        * CouplingA(j, k) * CouplingA(j, l) * CouplingA(k, l)

def CouplingCC(i, j, k, l):
    return CouplingC(i, j, k, l) * (waveNumber[i] + waveNumber[j] + waveNumber[k] + waveNumber[l])

def CouplingCCC(i, j, k, l):
    return CouplingC(i, j, k, l) * (waveNumber[i] + waveNumber[j] + waveNumber[k] + waveNumber[l])**2
...
def Numerator1(x, t):
    ...
    numerator += CouplingAAA(0, 1) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[1])
    numerator += CouplingAAA(0, 2) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[2])
    numerator += CouplingAAA(0, 3) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[3])
    numerator += CouplingAAA(1, 2) * np.exp(PhaseArg(x, t)[1] + PhaseArg(x, t)[2])
    numerator += CouplingAAA(1, 3) * np.exp(PhaseArg(x, t)[1] + PhaseArg(x, t)[3])
    numerator += CouplingAAA(2, 3) * np.exp(PhaseArg(x, t)[2] + PhaseArg(x, t)[3])

    numerator += CouplingBBB(0, 1, 2) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[1] + PhaseArg(x, t)[2])
    numerator += CouplingBBB(0, 1, 3) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[1] + PhaseArg(x, t)[3])
    numerator += CouplingBBB(0, 2, 3) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[2] + PhaseArg(x, t)[3])
    numerator += CouplingBBB(1, 2, 3) * np.exp(PhaseArg(x, t)[1] + PhaseArg(x, t)[2] + PhaseArg(x, t)[3])

    numerator += CouplingCCC(0, 1, 2, 3) * np.exp(sumPhaseArgs)
...
def Numerator2(x, t):
    ...
    numerator += CouplingAA(0, 1) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[1])
    numerator += CouplingAA(0, 2) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[2])
    numerator += CouplingAA(0, 3) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[3])
    numerator += CouplingAA(1, 2) * np.exp(PhaseArg(x, t)[1] + PhaseArg(x, t)[2])
    numerator += CouplingAA(1, 3) * np.exp(PhaseArg(x, t)[1] + PhaseArg(x, t)[3])
    numerator += CouplingAA(2, 3) * np.exp(PhaseArg(x, t)[2] + PhaseArg(x, t)[3])

    numerator += CouplingBB(0, 1, 2) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[1] + PhaseArg(x, t)[2])
    numerator += CouplingBB(0, 1, 3) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[1] + PhaseArg(x, t)[3])
    numerator += CouplingBB(0, 2, 3) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[2] + PhaseArg(x, t)[3])
    numerator += CouplingBB(1, 2, 3) * np.exp(PhaseArg(x, t)[1] + PhaseArg(x, t)[2] + PhaseArg(x, t)[3])

    numerator += CouplingCC(0, 1, 2, 3) * np.exp(sumPhaseArgs)
...
def Denominator(x, t):
    ...
    denominator += CouplingA(0, 1) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[1])
    denominator += CouplingA(0, 2) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[2])
    denominator += CouplingA(0, 3) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[3])
    denominator += CouplingA(1, 2) * np.exp(PhaseArg(x, t)[1] + PhaseArg(x, t)[2])
    denominator += CouplingA(1, 3) * np.exp(PhaseArg(x, t)[1] + PhaseArg(x, t)[3])
    denominator += CouplingA(2, 3) * np.exp(PhaseArg(x, t)[2] + PhaseArg(x, t)[3])

    denominator += CouplingB(0, 1, 2) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[1] + PhaseArg(x, t)[2])
    denominator += CouplingB(0, 1, 3) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[1] + PhaseArg(x, t)[3])
    denominator += CouplingB(0, 2, 3) * np.exp(PhaseArg(x, t)[0] + PhaseArg(x, t)[2] + PhaseArg(x, t)[3])
    denominator += CouplingB(1, 2, 3) * np.exp(PhaseArg(x, t)[1] + PhaseArg(x, t)[2] + PhaseArg(x, t)[3])

    denominator += CouplingC(0, 1, 2, 3) * np.exp(sumPhaseArgs)
...
waveNumber = np.array([2.2, 2.1, 1.9, 1.8])
phaseShift = np.array([-20, -15, 2, 10])
# TRIAL AND ERROR CORRECTIONS TO INITIAL POSITIONS (PEAK LOCATIONS) OF GHOST SOLITONS:
ghostInitialPositionCorrection = np.array([0.0, 3.6, 5.9, 9.425])
...
time = 23
print(f"Soliton max travel time = {SolitonMaxTravelTime()}")
...
y2 = [SingleSolitonGhost(xx, time, waveNumber[2], phaseShift[2] + ghostInitialPositionCorrection[2]) for xx in x]
y3 = [SingleSolitonGhost(xx, time, waveNumber[3], phaseShift[3] + ghostInitialPositionCorrection[3]) for xx in x]


latexExpression1 = r"""\parbox{20cm}{\center 
Four-Soliton Solution of KdV Equation:\\[1ex]
...
$\eta_i(x, t) = k_i \bigl( x - \underbrace{(k_i^2 - 6 a)}_{c_i \mathrm{\ (velocity)}} t - \delta_i \bigr), \quad i \in \{ 0, 1, 2, 3 \}, \quad t = %.1f$
\\[1ex] $u( \{ \eta_i(x, t) \} ) = \cdots + a\quad$ (a closed-form expression)
\\[1ex] $k_i = \{ %.2f, %.2f, %.2f, %.2f \}, \quad \delta_i = \{ %.2f, %.2f, %.2f, %.2f \}$
\\[1ex] $c_i = \{ %.2f, %.2f, %.2f, %.2f \}, \quad a = %.3f$ (integration const.)
}""".replace('\n',' ') % (time, waveNumber[0], waveNumber[1], waveNumber[2], waveNumber[3], phaseShift[0], phaseShift[1], phaseShift[2], phaseShift[3], SolitonVelocity()[0], SolitonVelocity()[1], SolitonVelocity()[2], SolitonVelocity()[3], integrationConst)
...
ax1.plot(x, y2, linestyle="dotted", linewidth=2, color="orange")
ax1.plot(x, y3, linestyle="dotted", linewidth=2, color="orange")
...
x2Labels = FixedFormatter([r"$\delta_0$", r"$\delta_1$", r"$\delta_2$", r"$\delta_3$"])
...
plt.savefig("Plots/FourSolitonCollision_t=" + str(time) + ".jpeg")
plt.show()
