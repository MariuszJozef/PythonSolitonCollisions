...
def Numerator1(x, t):
    ...
    numerator += CouplingBBB(0, 1, 2) * np.exp(sumPhaseArgs)
...
def Numerator2(x, t):
    ...
    numerator += CouplingBB(0, 1, 2) * np.exp(sumPhaseArgs)
...
def Denominator(x, t):
    ...
    denominator += CouplingB(0, 1, 2) * np.exp(sumPhaseArgs)
...
waveNumber = np.array([2.1, 2, 1.8])
phaseShift = np.array([-15, -3.7, 10])
# TRIAL AND ERROR CORRECTIONS TO INITIAL POSITIONS (PEAK LOCATIONS) OF GHOST SOLITONS:
ghostInitialPositionCorrection = np.array([0.0, 3.7, 6.125])
...
Three-Soliton Solution of KdV Equation:\\[1ex]
...
$\eta_i(x, t) = k_i \bigl( x - \underbrace{(k_i^2 - 6 a)}_{c_i \mathrm{\ (velocity)}} t - \delta_i \bigr), \quad i \in \{ 0, 1, 2 \}, \quad t = %.1f$
\\[1ex] $u( \{ \eta_i(x, t) \} ) = \cdots + a\quad$ (a closed-form expression)
\\[1ex] $k_i = \{ %.2f, %.2f, %.2f \}, \quad \delta_i = \{ %.2f, %.2f, %.2f \}$
\\[1ex] $c_i = \{ %.2f, %.2f, %.2f \}, \quad a = %.3f$ (integration const.)
}""".replace('\n',' ') % (time, waveNumber[0], waveNumber[1], waveNumber[2], phaseShift[0], phaseShift[1], phaseShift[2], SolitonVelocity()[0], SolitonVelocity()[1], SolitonVelocity()[2], integrationConst)
...
x2Labels = FixedFormatter([r"$\delta_0$", r"$\delta_1$", r"$\delta_2$"])
...
plt.savefig("Plots/ThreeSolitonCollision_t=" + str(time) + ".jpeg")
plt.show()
